...
MISSION="Mission 1.5: Clean House"
...
if [ "$(git rev-list --count "$BASE_REF"..HEAD 2>/dev/null || echo 0)" = "0" ]; then
    die "Something to submit" "Your branch has no changes for ARIA to review. Edit workspace/site.yml and run 'make test' until all phases pass, then run 'make submit' again"
fi
